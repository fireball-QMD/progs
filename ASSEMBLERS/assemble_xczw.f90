...
! Call the exchange-correlation interactions based on method chosen
           ...
           call average_rho (nprocs, Kscf, iforce, iordern, igauss)
            ...
            call assemble_zw_on_na (natoms, nprocs, my_proc, iordern,&
     &                                itheory)

            call assemble_zw_off_na (nprocs, my_proc, iordern, itheory)   

            call assemble_zw_2c_ct (nprocs, iforce, iordern)
...
!          Add assemble_lr here for the long long-range ewald contributions
           ...
           call assemble_zw_3c_ct (nprocs, iordern, igauss) 
