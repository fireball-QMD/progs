...
!	 This routine is modified denmat.f90 file to calculate density matrices
!and the band-structure energy in excited state.
!
! ===========================================================================
! Code rewritten by:
! James P. Lewis
...
! Program Declaration
! ===========================================================================
        subroutine denmat_es (ifixcharge, iqout, icluster, iwrtefermi, tempfe,  &
...
! Get the Fermi energy.
        ...
        if (itime_step_g .gt. 0 .and. flag_es .eq. 0 .and. gs_scf .eq. 1) then
         foccupy_na=foccupy
         ioccupy_na=ioccupy_k
        else if  (itime_step_g .gt. 0 .and. flag_es .eq. 0 .and. gs_scf .eq. 0) then
         foccupy=foccupy_na
         ioccupy_k=ioccupy_na
        end if
...
! ****************************************************************************
        ...
        if (iwrtpop .eq. 1 .and. itime_step_g .gt. 0 ) then
...
! Note: from principles it should be applied only to G k-point case
         ...
         open (unit = 35, file = 'populations_all.dat', status = 'unknown')
         write (35,*) ' # Charge localizations (per level): '
...
!            end do
            ...
            write (35,301) iband,eigen_k(iband,ikpoint),Wmu 
 !           Wmu_glob(iele)=Wmu
            Wmu_glob(iband)=Wmu
           ...
           if (icluster .ne. 1) then
            do iband = 1, norbitals_new
             if (ioccupy_na(iband,ikpoint) .ne. 0) then
              phase = phasex*foccupy_na(iband,ikpoint)
              do imu = 1, num_orb(in1)
               mmu = imu + degelec(iatom)
               step1 = phase*(bbnkre(mmu,iband,ikpoint)                      &
    &                        - ai*bbnkim(mmu,iband,ikpoint))
     ...
     &                         + ai*bbnkim(nnu,iband,ikpoint))
                gutr = real(step2)
! Finally the expressions.........
                rho_es(imu,inu,ineigh,iatom) = rho_es(imu,inu,ineigh,iatom) + gutr
                cape_es(imu,inu,ineigh,iatom) =                                 &
     &           cape_es(imu,inu,ineigh,iatom) + eigen_k(iband,ikpoint)*gutr
               end do
              end do
             end if
...
! Loop over all bands
     ...
     &                         + ai*bbnkim(nnu,iband,ikpoint))
                gutr = real(step2)
! Finally the expressions.........
                rhoPP_es(imu,inu,ineigh,iatom) = rhoPP_es(imu,inu,ineigh,iatom) &
     &                          + gutr
        ...
        if (iqout .ne. 2) then
         ...
         if (ifixcharge .eq. 1) then
          ...
          do iatom = 1, natoms
           in1 = imass(iatom)
           do issh = 1, nssh(in1)
            Qout_es(issh,iatom) = Qin(issh,iatom)
            QLowdin_TOT_es(iatom) = QLowdin_TOT_es(iatom) + Qin_es(issh,iatom)
           end do
          end do
...
! Loop over the special k points.
           do ikpoint = 1, nkpoints
            aux1 = weight_k(ikpoint)*spin
            do iorbital = 1, norbitals
             if (ioccupy_na(iorbital,ikpoint) .eq. 1) then
              aux2 = aux1*foccupy_na(iorbital,ikpoint)
...
! Finally the imu loop.
     ...
     &                        + blowim(mmu,iorbital,ikpoint)**2)
                ...
                Qout_es(issh,iatom) = Qout_es(issh,iatom) + aux3
                QLowdin_TOT_es(iatom) = QLowdin_TOT_es(iatom) + aux3
               end do
              end do
             end if
...
end if !if (itime_step .ne. 0) then
! ****************************************************************************
! END OF THE EXCITED STATE CALCULATIONS
