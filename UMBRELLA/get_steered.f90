...
! Program Declaration
! ===========================================================================
        subroutine get_steered (nstepi, nstepf, itime_step, time, natoms,   &
...
! contains all the information needed for the umbrella sampling MD runs.
        if (itime_step .eq. nstepi) then          ! for restart option...
         ...
         call readsteered (natoms, ratom, nstepf)
        endif
...
! this moment.
        call assemble_steered (umb_e, coord_reac_inter, itime_step, natoms, ratom)
...
! Umbrella sampling forces contributions
        ...
        write (*,*) '   ' 
        call Dassemble_steered (itime_step, natoms, iwrtfpieces, ratom, ftot)

        if (itime_step .ge. umb_time_start) then
         open (unit = 10, file = umb_output_file, status = 'unknown',        &
     &         position = 'append')
         write (10,*) coord_reac_inter(1:umb_pair), umb_e, etot_initial  
         close (unit = 10)
        end if
