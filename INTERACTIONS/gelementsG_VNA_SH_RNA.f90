...
! Program Declaration
! ===========================================================================
        subroutine gelementsG_VNA_SH_RNA(isorp,in1, in2, indna, l1,l2,  &
     &                                   issh, jssh, imu, jmu, y, rmatel)
! ===========================================================================
!
        use dimensions
        use interactions
...
! VNA by shell
!        real gcoefficientsVNA_SH(max_alphas,1:nsh_max,nspec_max)
!        real alphaVNA_SH(max_alphas,1:nsh_max,nspec_max)
!        integer nalphaVNA_SH(1:nsh_max,nspec_max)
...
! alpha2 (wavefunction)
        ...
        lilm = al1+al2
         ...
         smallQ = exp(-rmu*d12**2)
        smallgM = sqrt (pi/lilm)
...
! DD
     ...
     &             4.*c2*(xcm-x2)*(xcm-x1)
        ...
        rmatel = rmatel + factor*coef1*coef2*smallQ*smallgM**3
        end do
        end do

! Put in spherical harmonic type factors...
        rmatel = rmatel*gfactor(l1,imu)*gfactor(l2,jmu)/R_na(isorp,indna)
