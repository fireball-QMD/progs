...
!
     ...
     &                 iword,nfree,nact,ileave,nenter,iddot
      double precision theta,ddot,dr,rr,tol,my_dpmeps,                &
     &                 xstep,ddum,              &
     &                 cpu2,cachyt,sbtime,lnscht,time1,time2,      &
     &                 time !,dtd,gdold,stp,sbgnrm,stpmx,gd,cpu1,dnorm,fold
      double precision one,zero
!		because of ifort compiler - all warnings :-(
	double precision ::	 fold = 0
	double precision ::	 dnorm = 0
	double precision ::	 cpu1 = 0
	double precision ::	 gd = 0
	double precision ::	 stpmx = 0
	double precision ::	 sbgnrm = 0
	double precision ::	 stp = 0
	double precision ::	 gdold = 0
	double precision ::	 dtd = 0
!end chrom
...
!
!====================== The end of dnrm2 ===============================
      double precision function my_dpmeps()
!     the original function below make troubles (infinity loops).
!     precission calculated according http://en.wikipedia.org/wiki/Double_precision_floating-point_format#Double_precision_examples
!     (-1)^sign * 2^(exponent - exponent_bias) * 1.mantisa
!
!     This subroutine computes the machine precision parameter
!     dpmeps as the smallest floating point number such that
!     1 + 2**-n differs from 1. my_dpmeps = 2**-n
        double precision :: new_val

!        my_dpmeps = 0.000244140625 !0.5^12
        my_dpmeps = 1.0d0/1125899906842624.0 !0.5**50 - result should be ~ 0.5**52
        new_val = my_dpmeps * 0.5

        do while (1.0d0 < 1.0d0 + new_val)
            my_dpmeps = my_dpmeps * 0.5d0
            new_val = my_dpmeps * 0.5
        enddo
      end function my_dpmeps
