...
!
      ...
      complex, intent (in), dimension (norbitals,norbitals) :: alpha
...
!
      ...
      ELSE
         IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
            CALL CGERV2D( ICTXT, IB, JB, A( II+(JJ-1)*LDA ), LDA,  &
     &                    IRPRNT, ICPRNT )
         ELSE IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
            CALL CGESD2D( ICTXT, IB, JB, alpha (I, J), norbitals,  &
     &                    ICURROW, ICURCOL )
         END IF
      END IF
...
!
      DO 50 I = IN+1, IA+M-1, MB_A
         ...
         ELSE
            IF( MYROW.EQ.ICURROW .AND. MYCOL.EQ.ICURCOL ) THEN
               CALL CGERV2D( ICTXT, IB, JB, A( II+(JJ-1)*LDA ),  &
     &                       LDA, IRPRNT, ICPRNT )
            ELSE IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
               CALL CGESD2D( ICTXT, IB, JB, alpha(I, J), norbitals,  &
     &                       ICURROW, ICURCOL )
...
!
     ...
     &                          LDA, IRPRNT, ICPRNT )
               ELSE IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                  CALL CGESD2D( ICTXT, IB, JB, alpha(I, J),  &
