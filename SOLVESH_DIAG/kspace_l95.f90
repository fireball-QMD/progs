...
! Program Description
! ===========================================================================
!       This is a version of kspace.f that uses the LAPACK_95 library
! ===========================================================================
...
! ===========================================================================
     ...
     &                     nkpoints, iwrtdos, iwrthop, iwrtatom)
        ...
        use f95_lapack, only : la_heev, la_heevd
...
! A bunch of memory to be used in many ways
        ...
        complex(wp), dimension (:, :, :), allocatable, save :: sm12_save
...
! Call the diagonalizer
     ...
     &                   'V', 'U', info)
           ...
           call la_heev( zzzz(1:norbitals, 1:norbitals), slam(1:norbitals),  &
     &                   'V', 'U', info)
         end if
...
! after it is combined with overlap.
         ...
         call cgemm ('N', 'C', norbitals, norbitals, norbitals_new, a1,  &
    &                 zzzz, norbitals, zzzz, norbitals, a0, xxxx, norbitals)
...
! Eigenvectors are needed to calculate the charges and for forces!
        if (divide) then
          call la_heevd( yyyy(1:norbitals, 1:norbitals), eigen(1:norbitals),  &
     &                   'V', 'U', info)
!          call cheevd('V', 'U', norbitals, yyyy, norbitals, eigen, work,  & 
!     &                 lwork, rwork , lrwork, iwork, liwork, info ) 
        else
          call la_heev( yyyy(1:norbitals, 1:norbitals), eigen(1:norbitals),  &
     &                   'V', 'U', info)
