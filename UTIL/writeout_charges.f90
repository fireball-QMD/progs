...
! Write out the density for the first atom.
  ...
  if (iwrtcharges .eq. 1) then 
    ...
    if ((iqout .eq. 2) .or. (iqout .eq. 4)) then !Mullk y Mulliken-dipole
      write (*,*) '  '
      write (*,*) '  '
  ...
  if (iwrtcharges .eq. 3) then
    if (ab .eq. 1) then
      open(unit = 333, file = 'CHARGES.xyz', status = 'unknown', position = 'append')
       ...
       open(unit = 333, file = 'CHARGES_no_mix.xyz', status = 'unknown', position = 'append')
    end if ! if ab .eq. 1
    write(333,*) natoms
    ...
    do iatom = 1, natoms
      ...
      write(333,601) (Qout(issh,iatom), issh = 1, nssh(in1)), Qtot
    end do !end do iatom = 1,natoms
    close(333)     
  ...
  if (iwrtdipole .gt. 0) then
    ...
    if ( iwrtdipole .eq. 2) then
     !DIP RES = DIP_TOT - PROY
      ...
      open( unit = 800, file = 'PCHARGES', status ='unknown')
      do iatom = 1, natoms
