...
! average out small numerical noise and instabilities
     ...
     write(*,*), 'y symmetry averaging vertex point'

      denintemp = cmplx(0.0d0,0.0d0)
      denouttemp = cmplx(0.0d0,0.0d0)
      do jv = 1,Nvert
 !       if (mod(jv,1000) .eq. 0 ) then
!	    write(*,*), 'y symmetry averaging vertex point', jv
!	   end if
! mirror symmetry for y, -y reflection
        rrotvert(jv,1) = rvert(jv,1)
		rrotvert(jv,2) = -rvert(jv,2)
		rrotvert(jv,3) = rvert(jv,3)
        !find old vertex closest
        dalphamin = 2*pi !Make it way too big
       do kv = 1,Nvert !dalpha is approximate angular difference between vertex points
...
!      !Add to vertex point
     ...
     ! negative mirror symmetry for z, -z reflection
!      write(*,*), 'z symmetry averging vertex point', jv
!      denintemp = cmplx(0.0d0,0.0d0)
!      denouttemp = cmplx(0.0d0,0.0d0)
!      do jv = 1,Nvert
...
!      denouttemp = cmplx(0.0d0,0.0d0)
!      do jv = 1,Nvert
!        rrotvert(jv,1) = eps2(1,1)*rvert(jv,1) + eps2(1,2)*rvert(jv,2)
!		rrotvert(jv,2) = eps2(2,1)*rvert(jv,1) + eps2(2,2)*rvert(jv,2)
!		rrotvert(jv,3) = rvert(jv,3)
!        !find old vertex closest
!        dalphamin = 2*pi !Make it way too big
!       do kv = 1,Nvert !dalpha is approximate angular difference between vertex points
